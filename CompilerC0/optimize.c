...
void emitneworder(int bbidx,int btidx){
    ...
    for(p = list_head;p!=NULL;p = p->next){
        ...
        if(p->type==F_VAR&&p->va==GLV&&p->isact==0){
            int nextcode = bblocklist[bbidx].en;
            if(nextcode>=btab[btidx].code_en){
                continue;
            }
            switch(code4[nextcode].f){
                case SYS_PRINTCH:case SYS_PRINTINT:case RET:case PUSH://x
                case BEQ:case BNE:case BGE:case BGT:case BLE:case BLT:case ASNA://x,y
                case GETA://y,z
                    if(code4[nextcode].flag_x==F_VAR&&code4[nextcode].x==GLV){
                        if(p->node->varnodes!=NULL){
                            fx = p->node->varnodes->ele->type;
                            x = p->node->varnodes->ele->va;
                        }
                        else{
                            fx = F_TMPV;
                            x = p->node->index;
                        }
                        code4[nextcode].flag_x = fx;
                        code4[nextcode].x = x;
                    }
                    if(code4[nextcode].flag_y==F_VAR&&code4[nextcode].y==GLV){
                        if(p->node->varnodes!=NULL){
                            fy = p->node->varnodes->ele->type;
                            y = p->node->varnodes->ele->va;
                        }
                        else{
                            fy = F_TMPV;
                            y = p->node->index;
                        }
                        code4[nextcode].flag_y = fy;
                        code4[nextcode].y = y;
                    }
                    if(code4[nextcode].flag_z==F_VAR&&code4[nextcode].z==GLV){
                        if(p->node->varnodes!=NULL){
                            fz = p->node->varnodes->ele->type;
                            z = p->node->varnodes->ele->va;
                        }
                        else{
                            fz = F_TMPV;
                            z = p->node->index;
                        }
                        code4[nextcode].flag_z = fz;
                        code4[nextcode].z = z;
                    }
            }
        }
...
void renameActVar(int btidx){
    ...
    for(i = 0;i < bsc_blks;i ++){
        ...
        while(q){
            p = head;
            while(p){
                if(q->p->type==F_VAR&&p->o==q->p->var&&p->o>0){
                    break;
                }
                p = p->next;
            }
...
/*void createConfGraph(int btidx){
    active(btidx,2);
    int i,j;
    for(i = 0;i < btab[btidx].vd-1;i ++){
        for(j = 0;j < btab[btidx].vd-1;j ++){
